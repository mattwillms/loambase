...
| Admin / Ops Dashboard | **Mimus**    | mimus.willms.co      |
...
- **Resume-safe:** On each run, resumes from `current_page + 1` of the last failed run. Skips if last run is `complete` or currently `running`
- **Deduplication:** Upserts by `(source="perenual", external_id=str(perenual_id))` — no duplicates possible
- **Rate limit handling:** Raises `RateLimitError` on HTTP 429 or Perenual upgrade-notice body; marks run as `failed` and stops cleanly for next-day resumption
- **Free tier math:** 100 req/day → ~3 pages (90 list requests) + ~10 detail fetches per day. Full 3,000-species seed takes ~3 days
...
| **Notifications**             | ntfy.sh or Gotify    | Self-hosted, simple, works on phone.                                                 |
...
| ------------ | ------------------------------------------------------------------------------------------------------------- |
| Session 1–4  | Phase 1 backend foundation complete (see phase checklist)                                                     |
| This session | Signed up for Perenual free tier (Personal plan — 100 req/day, species 1–3000)                                |
| This session | Built and validated plant seeder end-to-end. 5 bugs found and fixed by Claude Code during live run.           |
| This session | `image_url` field changed from `String(500)` to `Text` — Perenual S3 URLs are 800+ chars                      |
| This session | Confirmed ARQ (not Celery) is the task queue in use — spec updated to remove Celery references                |
| This session | Scripts in `~/dev/loambase/scripts/` are not auto-mounted in containers — `docker cp` or update compose mount |
| This session | Container naming pattern confirmed: `loambase-loambase-api-1`, `loambase-loambase-worker-1`                   |
| This session | After `.env` changes, restart both worker and API: `docker-compose restart loambase-worker loambase-api`      |
